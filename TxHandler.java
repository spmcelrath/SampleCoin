...
public class TxHandler {
    ...
    public boolean isValidTx(Transaction tx) {
      ...
      for(int i = 0; i < tx.numInputs(); i++){
        ...
        if(!Crypto.verifySignature(output.address, tx.getRawDataToSign(i), in.signautre)) return false;
        ...
        for(Transaction tx : possibleTxs){
          if(isValidTx(tx)){
            ...
            for(Transaction.Input in : tx.getInputs()){
              UXTO uxto = new UXTO(in.prevTxHash, in.outputIndex);
              uxtoPool.removeUTXO(utxo, out);
            }
          }
        }
